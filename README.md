# Hi there, I'm Farhan Kholid 👋

![Banner](https://raw.githubusercontent.com/amaheeeen/amaheeeen/main/BG-DEPAN.png)

## 🚀 Front-End Engineer Associate | UI/Visual Designer
I am a hybrid professional with a strong background in visual design and branding (4 years), currently actively transitioning into **Front-End Engineering**. I thrive on translating complex ideas into elegant user interfaces and powerful visual assets.

### 🛠️ Technical Ecosystem
- **Languages:** JavaScript (ES6+), PHP (Laravel), HTML5, CSS3, TypeScript.
- **Frameworks/Libraries:** React.js, Next.js, Tailwind CSS, Shadcn/UI.
- **Tools:** Git/GitHub, Figma, Adobe Creative Suite, HTML5 Canvas API.

### 🌟 Featured Projects
- **[Mono Stock](https://github.com/frhnkhld/Mono-Inventory-Dashboard):** Minimalist Inventory Dashboard built with Vanilla JS & Canvas API.
- **[Kalcer.id](https://github.com/frhnkhld/portofolio-farhan):** Viral Hangout Monitoring System built with Laravel.
- **[DataSmoothing](https://github.com/amaheeeen/DataSmoothing):** Interactive Time Series visualization tool.

### 📫 Let's Connect
- **Portfolio:** [Visit my website](https://amaheeeen.github.io)
- **LinkedIn:** [farhan-kholid](https://www.linkedin.com/in/farhan-kholid-8115b9202/)
- **Email:** farhankholid20@gmail.com

---
*“Crafting digital experiences that are intuitive, functional, and aesthetic.”*
